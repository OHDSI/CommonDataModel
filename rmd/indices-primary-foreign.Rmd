...
## Overview
Primary and foreign key constraints Database indices improve the performance of queries against a database by organizing the data in a way that increase query execution.
...
The scripts to deploy the primary and foreign key constraints can be found in the ddl/[version]/[dialect] directory and should be run once data has been loaded as a data integrity check.  
...
The scripts to deploy the indices can be found in the ddl/[version]/[dialect] directory and should be run once the data has been loaded AND foreign / primary key constraints have been run.

## Database support
The OHDSI/CommonDataModel package leverages OHDSI/SQLRender and as a result is only capable of supporting sources that are supported by [OHDSI/SQLRender](https://ohdsi.github.io/SqlRender/).
...
### Databricks
This database type is not yet supported but is actively being worked on by a number of collaborators.  More information coming soon.
