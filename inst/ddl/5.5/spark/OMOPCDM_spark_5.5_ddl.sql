...
CAST(NULL AS integer) AS measurement_id,
	...
	CAST(NULL AS integer) AS value_as_source_concept_id,
...
CAST(NULL AS integer) AS observation_id,
	...
	CAST(NULL AS integer) AS value_as_concept_id,
	IF(try_cast(NULL  AS DATE) IS NULL, to_date(cast(NULL  AS STRING), 'yyyyMMdd'), try_cast(NULL  AS DATE)) AS value_as_date,
...
CAST(NULL AS integer) AS specimen_id,
	...
	CAST(NULL AS integer) AS visit_occurrence_id,
	CAST(NULL AS integer) AS visit_detail_id  WHERE 1 = 0;
...
CAST(NULL AS STRING) AS cdm_source_name,
	...
	CAST(NULL AS STRING) AS source_release_version,
...
AS
SELECT
CAST(NULL AS integer) AS pack_concept_id,
	CAST(NULL AS integer) AS drug_concept_id,
	CAST(NULL AS integer) AS amount,
	CAST(NULL AS integer) AS box_size  WHERE 1 = 0;
OPTIMIZE @cdmDatabaseSchema.pack_content   ZORDER BY RANDOM;
CREATE TABLE @cdmDatabaseSchema.concept_metadata  
USING DELTA
AS
SELECT
CAST(NULL AS integer) AS concept_id,
	CAST(NULL AS STRING) AS concept_category,
	CAST(NULL AS STRING) AS reuse_status  WHERE 1 = 0;
OPTIMIZE @cdmDatabaseSchema.concept_metadata   ZORDER BY RANDOM;
...
CAST(NULL AS integer) AS concept_id_1,
	CAST(NULL AS integer) AS concept_id_2,
	CAST(NULL AS STRING) AS relationship_id,
	CAST(NULL AS STRING) AS relationship_predicate_id,
	CAST(NULL AS integer) AS relationship_group,
	CAST(NULL AS STRING) AS mapping_source,
	CAST(NULL AS DOUBLE) AS confidence,
	CAST(NULL AS STRING) AS mapping_tool,
	CAST(NULL AS STRING) AS mapper,
	CAST(NULL AS STRING) AS reviewer  WHERE 1 = 0;
OPTIMIZE @cdmDatabaseSchema.concept_relationship_metadata   ZORDER BY RANDOM;
