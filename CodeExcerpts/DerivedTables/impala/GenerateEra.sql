...
d.DRUG_EXPOSURE_ID
	...
	,COALESCE(DRUG_EXPOSURE_END_DATETIME, DATE_ADD(CASE TYPEOF(DRUG_EXPOSURE_START_DATETIME ) WHEN 'TIMESTAMP' THEN CAST(DRUG_EXPOSURE_START_DATETIME  AS TIMESTAMP) ELSE TO_UTC_TIMESTAMP(CONCAT_WS('-', SUBSTR(CAST(DRUG_EXPOSURE_START_DATETIME  AS STRING), 1, 4), SUBSTR(CAST(DRUG_EXPOSURE_START_DATETIME  AS STRING), 5, 2), SUBSTR(CAST(DRUG_EXPOSURE_START_DATETIME  AS STRING), 7, 2)), 'UTC') END, DAYS_SUPPLY), DATE_ADD(CASE TYPEOF(DRUG_EXPOSURE_START_DATETIME ) WHEN 'TIMESTAMP' THEN CAST(DRUG_EXPOSURE_START_DATETIME  AS TIMESTAMP) ELSE TO_UTC_TIMESTAMP(CONCAT_WS('-', SUBSTR(CAST(DRUG_EXPOSURE_START_DATETIME  AS STRING), 1, 4), SUBSTR(CAST(DRUG_EXPOSURE_START_DATETIME  AS STRING), 5, 2), SUBSTR(CAST(DRUG_EXPOSURE_START_DATETIME  AS STRING), 7, 2)), 'UTC') END, 1)) AS DRUG_EXPOSURE_END_DATETIME
...
WHERE c.VOCABULARY_ID = 'RxNorm'
 ...
 COMPUTE STATS n7cbw799cteDrugTarget
;
...
PERSON_ID
	...
	,DATE_ADD(CASE TYPEOF(EVENT_DATETIME ) WHEN 'TIMESTAMP' THEN CAST(EVENT_DATETIME  AS TIMESTAMP) ELSE TO_UTC_TIMESTAMP(CONCAT_WS('-', SUBSTR(CAST(EVENT_DATETIME  AS STRING), 1, 4), SUBSTR(CAST(EVENT_DATETIME  AS STRING), 5, 2), SUBSTR(CAST(EVENT_DATETIME  AS STRING), 7, 2)), 'UTC') END, - 30) AS END_DATETIME -- unpad the end date
...
(
	...
	FROM (
		...
		FROM (
			...
			SELECT PERSON_ID
				,INGREDIENT_CONCEPT_ID
				,DATE_ADD(CASE TYPEOF(DRUG_EXPOSURE_END_DATETIME ) WHEN 'TIMESTAMP' THEN CAST(DRUG_EXPOSURE_END_DATETIME  AS TIMESTAMP) ELSE TO_UTC_TIMESTAMP(CONCAT_WS('-', SUBSTR(CAST(DRUG_EXPOSURE_END_DATETIME  AS STRING), 1, 4), SUBSTR(CAST(DRUG_EXPOSURE_END_DATETIME  AS STRING), 5, 2), SUBSTR(CAST(DRUG_EXPOSURE_END_DATETIME  AS STRING), 7, 2)), 'UTC') END, 30)
				,1 AS EVENT_TYPE
				,NULL
			FROM n7cbw799cteDrugTarget
			) RAWDATA
...
WHERE 2 * E.START_ORDINAL - E.OVERALL_ORD = 0;
 COMPUTE STATS n7cbw799cteEndDates
;
...
GROUP BY d.PERSON_ID
	...
	,d.DRUG_TYPE_CONCEPT_ID
	,d.DRUG_EXPOSURE_START_DATETIME;
 COMPUTE STATS n7cbw799cteDrugExpEnds
;
...
co.PERSON_ID
	...
	,COALESCE(co.CONDITION_END_DATETIME, DATE_ADD(CASE TYPEOF(CONDITION_START_DATETIME ) WHEN 'TIMESTAMP' THEN CAST(CONDITION_START_DATETIME  AS TIMESTAMP) ELSE TO_UTC_TIMESTAMP(CONCAT_WS('-', SUBSTR(CAST(CONDITION_START_DATETIME  AS STRING), 1, 4), SUBSTR(CAST(CONDITION_START_DATETIME  AS STRING), 5, 2), SUBSTR(CAST(CONDITION_START_DATETIME  AS STRING), 7, 2)), 'UTC') END, 1)) AS CONDITION_END_DATETIME

FROM
[CDM].[CDMSCHEMA].CONDITION_OCCURRENCE co;
 COMPUTE STATS n7cbw799cteConditionTarget
;
...
(
	...
	FROM (
		...
		FROM (
			...
			SELECT PERSON_ID
				,CONDITION_CONCEPT_ID
				,DATE_ADD(CASE TYPEOF(CONDITION_END_DATETIME ) WHEN 'TIMESTAMP' THEN CAST(CONDITION_END_DATETIME  AS TIMESTAMP) ELSE TO_UTC_TIMESTAMP(CONCAT_WS('-', SUBSTR(CAST(CONDITION_END_DATETIME  AS STRING), 1, 4), SUBSTR(CAST(CONDITION_END_DATETIME  AS STRING), 5, 2), SUBSTR(CAST(CONDITION_END_DATETIME  AS STRING), 7, 2)), 'UTC') END, 30)
				,1 AS EVENT_TYPE
				,NULL
			FROM n7cbw799cteConditionTarget
			) RAWDATA
...
WHERE (2 * E.START_ORDINAL) - E.OVERALL_ORD = 0;
 COMPUTE STATS n7cbw799cteCondEndDates
;
...
GROUP BY c.PERSON_ID
	,c.CONDITION_CONCEPT_ID
	,c.CONDITION_START_DATETIME;
 COMPUTE STATS n7cbw799cteConditionEnds
;
