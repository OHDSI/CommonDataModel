...
d.DRUG_EXPOSURE_ID
	...
	,COALESCE(DRUG_EXPOSURE_END_DATETIME, DATEADD(day,CAST(DAYS_SUPPLY as int),DRUG_EXPOSURE_START_DATETIME), DATEADD(day,CAST(1 as int),DRUG_EXPOSURE_START_DATETIME)) AS DRUG_EXPOSURE_END_DATETIME
...
CREATE TABLE #cteEndDates

DISTKEY(person_id)
AS
SELECT
 person_id ,INGREDIENT_CONCEPT_ID
	,DATEADD(day,CAST(- 30 as int),EVENT_DATETIME) AS END_DATETIME -- unpad the end date
...
(
	...
	FROM (
		...
		FROM (
			...
			SELECT PERSON_ID
				,INGREDIENT_CONCEPT_ID
				,DATEADD(day,CAST(30 as int),DRUG_EXPOSURE_END_DATETIME)
				,1 AS EVENT_TYPE
				,NULL
			FROM #cteDrugTarget
			) RAWDATA
...
CREATE TABLE #cteConditionTarget
...
co. person_id ,co.condition_concept_id
	...
	,COALESCE(co.CONDITION_END_DATETIME, DATEADD(day,CAST(1 as int),CONDITION_START_DATETIME)) AS CONDITION_END_DATETIME
...
(
	...
	FROM (
		...
		FROM (
			...
			SELECT PERSON_ID
				,CONDITION_CONCEPT_ID
				,DATEADD(day,CAST(30 as int),CONDITION_END_DATETIME)
				,1 AS EVENT_TYPE
				,NULL
			FROM #cteConditionTarget
			) RAWDATA
