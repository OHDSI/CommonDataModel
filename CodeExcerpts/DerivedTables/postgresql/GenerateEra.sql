...
d.DRUG_EXPOSURE_ID
	...
	,COALESCE(DRUG_EXPOSURE_END_DATETIME, (DRUG_EXPOSURE_START_DATETIME + DAYS_SUPPLY*INTERVAL'1 day'), (DRUG_EXPOSURE_START_DATETIME + 1*INTERVAL'1 day')) AS DRUG_EXPOSURE_END_DATETIME
...
PERSON_ID
	...
	,(EVENT_DATETIME + - 30*INTERVAL'1 day') AS END_DATETIME -- unpad the end date
...
(
	...
	FROM (
		...
		FROM (
			...
			SELECT PERSON_ID
				,INGREDIENT_CONCEPT_ID
				,(DRUG_EXPOSURE_END_DATETIME + 30*INTERVAL'1 day')
				,1 AS EVENT_TYPE
				,NULL
			FROM cteDrugTarget
			) RAWDATA
...
co.PERSON_ID
	...
	,COALESCE(co.CONDITION_END_DATETIME, (CONDITION_START_DATETIME + 1*INTERVAL'1 day')) AS CONDITION_END_DATETIME
...
(
	...
	FROM (
		...
		FROM (
			...
			SELECT PERSON_ID
				,CONDITION_CONCEPT_ID
				,(CONDITION_END_DATETIME + 30*INTERVAL'1 day')
				,1 AS EVENT_TYPE
				,NULL
			FROM cteConditionTarget
			) RAWDATA
